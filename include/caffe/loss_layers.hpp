...
template <typename Dtype>
class TripletSimilarityPrecisionLayer : public Layer<Dtype> {
 public:
  explicit TripletSimilarityPrecisionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_TRIPLET_SIMILARITY_PRECISION;
  }
